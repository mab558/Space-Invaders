...
class Ship(GImage):
    ...
    def __init__(self):
        ...
        A ship is a subclass of GImage and, hence, contains all of the
        attributes of GImage. The ship is initialized according to given
        constants.
        """
    ...
    def collides(self,bolt):
        """
        Return True if an alien bolt collides with the ship

        Parameter bolt: the laser bolt currently on screen
        Precondition: bolt is a Bolt object, possibly None
        """
        if not(bolt.isPlayerBolt()):
            if self.contains((bolt.left,bolt.bottom)):
                return True
            elif self.contains ((bolt.right,bolt.bottom)):
                return True
            elif self.contains ((bolt.left,bolt.top)):
                return True
...
class Alien(GImage):
    ...
    def __init__(self,left,top,z):
        """
        Initializes an alien

        Alien is a GImage with a source from ALIEN_IMAGES

        Parameter left: the horizontal coordinate of the left edge
        Precondition: left must be an int or float

        Paramenter top: the vertical coordinate of the top edge
        Precondition: top must be an int or float

        Parameter z: the position of the source in ALIEN_IMAGES
        Precondition: z must be an int < length of ALIEN_IMAGES
        """
        assert isinstance(left,float) or isinstance(left,int)
        assert isinstance(top,float) or isinstance(top,int)
        assert isinstance(z,int) and z < len(ALIEN_IMAGES)

        super().__init__(left=left,top=top,width=ALIEN_WIDTH,
        height=ALIEN_HEIGHT,source=ALIEN_IMAGES[z])


    # METHOD TO CHECK FOR COLLISION (IF DESIRED)
    def collides(self,bolt):
        """
        Return True if a player bolt collides with the alien
...
class Bolt(GRectangle):
    ...
    def __init__ (self,x,y,velocity):
        """
        Initializes a bolt with x position as x and y position as y with a
        specific velocity given for moving

        Parameter x: horizontal coordinate of the object center
        Precondition: x is an int or float

        Parameter y: vertical coordinate of the object center
        ...
        super().__init__(x=x, y=y, fillcolor='red',
    ...
    def isPlayerBolt(self):
        """
        Return True if the bolt is a player bolt

        Bolts with positive velocity are player bolts
        """
        if self._velocity >= 0:
            return True
        else:
